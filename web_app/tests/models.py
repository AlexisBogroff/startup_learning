...
class DynMCQInfo(models.Model):
    ...
    questions (string) : list of id questions of the test (a[1,4,6]b[8,1,2]
    => a is for QCM questions and b for normal questions)
    ...
    def get_absolute_url(self):
        return reverse('tests:Create DynMCQTest',
                       kwargs={'input_id_test': self.id_test})

    def get_absolute_url_q_menu(self):
        return reverse('tests:SelectMenu DynMCQquestion',
                       kwargs={'input_id_test': self.id_test})

    def get_absolute_url_display(self):
        return reverse('tests:Display DynMCQtest',
                       kwargs={'input_id_test': self.id_test})

    def question_reallocation(self):
        return reverse('tests:Question_reallocation',
                       kwargs={'input_id_test': self.id_test})

    def get_statistics(self):
        return reverse('tests:Statistics',
                       kwargs={'input_id_test': self.id_test})
    ...
    def get_launch(self):
        return reverse('tests:Launch Specific McqDyn',
                       kwargs={'input_id_test': self.id_test})

    def get_in_launch(self):
        print_test = True
        return reverse('tests:In Launch Specific DynMcq',
                       kwargs={'input_id_test': self.id_test})

    def stop_launch(self):
        return reverse('tests:Stop mcq launch',
                       kwargs={'input_id_test': self.id_test})


class Dynquestion(models.Model):
    """Model of the first type of question, the normal question
    with text answer :
    ...
    difficulty (string) : list of difficulties depending to themes ([2,0,4,0,1]
    => difficulty 2 for theme 1, difficulty 0 for theme 2 ...)

    Function linked to the model :

    get_absolute_url_question : render the page to create the question
    get_absolute_url_difficulty : render the page to add the difficulty
    to the question
    ...
    def get_absolute_url_question(self):
        return reverse('tests:Create Dynquestion',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_difficulty(self):
        return reverse('tests:Add Difficulty question',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_edit(self):
        return reverse('tests:Edit Dynquestion',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_delete(self):
        return reverse('tests:Delete Dynquestion',
                       kwargs={'input_q_num': self.q_num})


class DynMCQquestion(models.Model):
    """Model of the second type of question, the MCQ question
    ...
    get_absolute_url_question : render the page to create the question
    get_absolute_url_answers : render the page to create the answers
    of the question
    get_absolute_url_difficulty : render the page to add the difficulty
    to the question
    get_absolute_url_edit : render the page to edit the question
    get_absolute_url_delete : render the page to delete the question
    get_absolute_url_add_answer : render the page to add an answer
    to the question
    """
    q_num = models.AutoField(primary_key=True)
    q_text = models.TextField()
    ...
    def get_absolute_url_question(self):
        return reverse('tests:Create DynMCQquestion',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_answers(self):
        return reverse('tests:Create DynMCQanswers',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_difficulty(self):
        return reverse('tests:Add Difficulty',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_edit(self):
        return reverse('tests:Edit DynMCQquestion',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_delete(self):
        return reverse('tests:Delete DynMCQquestion',
                       kwargs={'input_q_num': self.q_num})

    def get_absolute_url_add_answer(self):
        return reverse('tests:AddAnswer DynMCQanswer',
                       kwargs={'input_q_num': self.q_num})
...
class DynMCQanswer(models.Model):
    ...
    def get_absolute_url_edit(self):
        return reverse('tests:Edit DynMCQanswer',
                       kwargs={'input_q_num': self.q_num,
                               'input_ans_num': self.ans_num})

    def get_absolute_url_delete(self):
        return reverse('tests:Delete DynMCQanswer',
                       kwargs={'input_q_num': self.q_num,
                               'input_ans_num': self.ans_num})


class PassDynMCQTestInfo(models.Model):
    """Main model for test answers of the user :
    PassDynMCQTestInfo gather the general informations on the test.
    ...
    def get_absolute_url(self):
        return reverse('tests:Pass dynmcqtest',
                       kwargs={'input_id_test': self.id_test,
                               'input_id_student': self.id_student,
                               'input_attempt': self.attempt})

    def get_absolute_url_display(self):
        return reverse('tests:Display pass dynmcqtest',
                       kwargs={'input_id_test': self.id_test,
                               'input_id_student': self.id_student,
                               'input_attempt': self.attempt})


class PassDynMCQTest(models.Model):
    ...
    r_ans (string) : list of number of the answers ([1,2]
    when checking first and second answer)
    """
    id_test = models.CharField(max_length=10, null=True)
    id_student = models.CharField(max_length=10, null=True)
    attempt = models.IntegerField(null=True)
...
class PassDynquestionTest(models.Model):
