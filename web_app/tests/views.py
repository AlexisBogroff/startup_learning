...
from .forms import DynMCQquestionForm, DynMCQanswerForm, PassDynMCQTestForm, \
    DynMCQquestionFormQuestion, DynMCQTestInfoForm, \
    DynMCQTestInfoFormQuestions, QuestionDifficultyForm, \
    MCQQuestionDifficultyForm, DynMCQTestInfoFormlaunch, DynquestionForm, \
    PassDynquestionTestForm
from .models import DynMCQInfo, DynMCQquestion, DynMCQanswer, PassDynMCQTest, \
    PassDynMCQTestInfo, Dynquestion, PassDynquestionTest


def register_view(request):
    """Function to create a user on the app :
    Render the register page and retreive the informations
    of the user via the POST method
    ...
    if request.method == 'POST':
        ...
        # we display an error message and we refresh the page
...
def login_view(request):
    ...
    Render the login page and retreive the informations
     of the user via the POST method
    ...
    if request.method == 'POST':
        ...
        # If the user exists, we login him
        # and then returns the main page of the site
        if user is not None:
            login(request, user)
            return redirect('/')
...
def DynMCQquestion_select_menu_view(request, input_id_test):
    """Function to put questions in the test if there are not questions
    in the test
    ...
    if (DynMCQTestInfo.questions != ""):
        num_questions = get_questions(
            DynMCQTestInfo.questions)  # Get id number of the questions
    ...
    form = []
    # If there are not questions in the test, we display the form
    # to put questions in the test
    if len(DynMCQquestionTestList) == 0 and len(DynquestionTestList) == 0:
        empty = True
        QuestionsSet = formset_factory(DynMCQTestInfoFormQuestions,
                                       extra=2)
        # Two form for both question types
        ...
        DynMCQquestions = DynMCQquestion.objects.all()
        Dynquestions = Dynquestion.objects.all()
        # Filling the choice for DynMCQquestion with [q_num,q_text]
        # for each questions
        # (q_text is displayed and q_num is the returned value)
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                DynMCQTestInfo.questions = "a" \
                                           + test.questions \
                                           + "b" \
                                           + test2.questions
                # Save the questions as
                # "a[MCQ_ids,...]b[Normal_question_ids,...]" in the test
                DynMCQTestInfo.save()
                ...
                normal_questions = num_questions[1]
                for i in range(len(mcq_questions)):
                    DynMCQQuestion = DynMCQquestion.objects.get(
                        q_num=mcq_questions[i])
                    DynMCQquestionTestList.append(DynMCQQuestion)
                for i in range(len(normal_questions)):
                    DynQuestion = Dynquestion.objects.get(
                        q_num=normal_questions[i])
                    DynquestionTestList.append(DynQuestion)
...
def Question_reallocation_view(request, input_id_test):
    ...
    QuestionsSet = formset_factory(DynMCQTestInfoFormQuestions, extra=2)
    # Three mandatory properties for formset
    data = {
        'form-TOTAL_FORMS': 2,
        'form-INITIAL_FORMS': '0',
    ...
    Dynquestions = Dynquestion.objects.all()
    # Filling the choice for DynMCQquestion with [q_num,q_text]
    # for each questions (q_text is displayed and q_num is the returned value)
    for question in DynMCQquestions:
        list = []
        list.append(int(question.q_num))
        list.append(question.q_text)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            DynMCQTestInfo.questions = "a" \
                                       ...
                                       + test2.questions
            # Save the questions
            # as "a[MCQ_ids,...]b[Normal_question_ids,...]" in the test
            DynMCQTestInfo.save()
...
def get_questions(questions):
    ...
    Parameter :
        questions (str) : questions of the test for example :
        a[1,5,6]b[1,9,11] or a[4,5]b or ab[4,9,7]
    Returns :
        num_questions (list) : list of two list,
        num_questions[0] contains id_questions for MCQquestions
        and num_questions[1] contains id_questions for Normal questions
    """
    num_questions = []
    # Case where there are both MCQ and normal questions
    # in the test so questions as a[1,5,6]b[1,9,11]
    if "]b[" in questions:
        ...
        list = []
        # Case where there are only normal questions
        # in the test so questions as ab[4,9,7]
        if (questions[posA + 1] == "b"):
            num_questions.append(list)
            num_questions.append(listquestions)
        # Case where there are only MCQ questions
        # in the test so questions as a[4,5]b
        else:
            num_questions.append(listquestions)
            num_questions.append(list)
    return num_questions
    # Returns [[1,5,6],[1,9,11]], [[],[4,9,7]], [[4,5],[]]
    # in each different cases
...
def DynMCQquestion_create_view(request, input_q_num):
    ...
    # If the question is filled, we check if there are answers
    # in the questions to display it
    else:
        empty_question = False
    ...
    return render(request, 'manage_tests/test_create_dynmcqquestion.html',
                  context)
...
def Add_difficulty_view(request, input_q_num):
    ...
    DynMCQquestionTest = get_object_or_404(DynMCQquestion, q_num=input_q_num)
    empty = True
    DifficultySet = formset_factory(MCQQuestionDifficultyForm, extra=5)
    # Three mandatory properties for formset
    data = {
        'form-TOTAL_FORMS': 5,
        'form-INITIAL_FORMS': '0',
    ...
    for theme in form:
        ...
        for i in range(5):
            ...
            choices.append(list)
        theme.fields[
            'difficulty'].choices = choices
        # Filling the choices for theme/difficulty as [theme,difficulty]
    ...
    if request.method == 'POST':
        ...
        for theme in form:
            ...
            for i in range(5):
                ...
                choices.append(list)
            theme.fields[
                'difficulty'].choices = choices  # Assignation des choix
...
def Add_difficulty_question_view(request, input_q_num):
    ...
    DynquestionTest = get_object_or_404(Dynquestion, q_num=input_q_num)
    empty = True
    DifficultySet = formset_factory(QuestionDifficultyForm, extra=5)
...
def DynMCQanswer_create_view(request, input_q_num):
    ...
    # If there are not answers in the test,
    # we display the formset of the answers
    if empty_answer == True:
        # We crate a formset of nb_answers forms of DynMCQanswerForm
        DynMCQanswerSet = formset_factory(DynMCQanswerForm,
                                          extra=int(nb_answers))
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                for instance in form:
                    ...
                    DynMCQanswerTest = DynMCQanswer.objects.filter(
                        q_num=input_q_num)
                    DynMCQanswerTest_List = []
                    for instance in DynMCQanswerTest:
                        DynMCQanswerTest_List.append(instance)
    ...
    return render(request, 'manage_tests/test_create_dynmcqanswer.html',
                  context)
...
def DynMCQTest_pass_menu_view(request, input_id_test):
    ...
    Pass_DynMCQInfo_filter = PassDynMCQTestInfo.objects.filter(
        id_test=input_id_test, id_student=request.user.username)
    ...
    if nb_pass_test > 0:
        ...
        if PassDynMCQTest.objects.filter(id_test=input_id_test,
                                         id_student=request.user.username,
                                         attempt=nb_pass_test).exists() \
                or PassDynquestionTest.objects.filter(
            id_test=input_id_test, id_student=request.user.username,
            attempt=nb_pass_test).exists():
            ...
            Pass_DynMCQInfo = get_object_or_404(PassDynMCQTestInfo,
                                                id_test=input_id_test,
                                                id_student=
                                                request.user.username,
                                                attempt=nb_pass_test)
            empty_passdynMCQtest = False

    if empty_passdynMCQtest:
        # We create a new attempt for passing the test
        passdynmcqtest = PassDynMCQTestInfo(id_test=input_id_test,
                                            id_student=request.user.username,
                                            attempt=nb_pass_test + 1, mark=0)
        passdynmcqtest.save()
        empty_passdynMCQtest = False
        Pass_DynMCQInfo = get_object_or_404(PassDynMCQTestInfo,
                                            id_test=input_id_test,
                                            id_student=request.user.username,
                                            attempt=nb_pass_test + 1)
...
def DynMCQtest_pass_view(request, input_id_test, input_id_student,
    ...
    if (nb_mcq_questions > 0):
        # We create a formset of nb_mcq_questions of PassDynMCQTestForm
        PassDynMCQTestSet = formset_factory(PassDynMCQTestForm,
                                            extra=int(nb_mcq_questions))
        # Three mandatory properties for formset
        data = {
        ...
        # We get the answers of each questions
        # and we put them in the choices for checkbox
        i = 0
        for ans in form_mcq_answers:
            choices = []
            DynMCQanswers = DynMCQanswer.objects.filter(q_num=mcq_questions[i])
            # Get the answers in list ('returned value','displayed text')
            # for each answers
            for answer in DynMCQanswers:
                ...
                choices.append(list)
            ans.fields[
                'r_ans'].choices = choices
            # Filling the choices as list of [ans_num,ans_text]
            i += 1
        ...
        if request.method == 'POST':
            ...
            for ans in form_mcq_answers:
                choices = []
                DynMCQanswers = DynMCQanswer.objects.filter(
                    q_num=mcq_questions[i])
                for answer in DynMCQanswers:
                    list = []
                    list.append(answer.ans_num)
            ...
            if form_mcq_answers.is_valid():
                ...
                for instance in form_mcq_answers:
                    ...
                    right_answers = DynMCQanswer.objects.filter(
                        q_num=mcq_questions[question_count], right_ans=1)
                    num_right_answers = []
                ...
                if double_type_question is False:
                    ...
                    time_left = compare_date(
                        get_date(str(datetime.datetime.today())), limit_time)
                    # If time left > 30 sec we put a penalty
                    # of 1 point every minute
                    if time_left < -0.5:
    ...
    if (nb_normal_questions > 0):
        # We create a formset of nb_normal_questions of PassDynquestionTestForm
        PassDynquestionTestSet = formset_factory(PassDynquestionTestForm,
                                                 extra=int(nb_normal_questions))
        # Three mandatory properties for formset
        data = {
            ...
            if form_normal_answers.is_valid():
                ...
                for instance in form_normal_answers:
                    if question_count < len(normal_questions):
                        ...
                        question = Dynquestion.objects.get(
                            q_num=normal_questions[question_count])
...
def pass_dynMCQtest_display_view(request, input_id_test, input_id_student,
    ...
    Pass_DynMCQInfo = get_object_or_404(PassDynMCQTestInfo,
                                        ...
                                        id_student=input_id_student,
                                        attempt=input_attempt)
    Pass_DynMCQtest = PassDynMCQTest.objects.filter(id_test=input_id_test,
                                                    id_student=
                                                    input_id_student,
                                                    attempt=input_attempt)
    Pass_Dynquestiontest = PassDynquestionTest.objects.filter(
        id_test=input_id_test, id_student=input_id_student,
        attempt=input_attempt)
...
def DynMCQtest_display_view(request, input_id_test):
    ...
    DynMCQanswers = []
    for i in range(len(mcq_questions)):
        theDynMCQanswers = DynMCQanswer.objects.filter(
            q_num=int(mcq_questions[i]))
        for ans in theDynMCQanswers:
            DynMCQanswers.append(ans)

    # We order the questions and the answers in a same list
...
def Edit_DynMCQquestion_view(request, input_q_num):
    ...
    form = DynMCQquestionFormQuestion(request.POST,
                                      instance=DynMCQquestionTest)
    if form.is_valid():
        form.save()
        form = DynMCQquestionFormQuestion(instance=DynMCQquestionTest)
...
def Edit_DynMCQanswer_view(request, input_q_num, input_ans_num):
    ...
    DynMCQquestionTest = get_object_or_404(DynMCQquestion, q_num=input_q_num)
    DynMCQanswerTest = get_object_or_404(DynMCQanswer, q_num=input_q_num,
                                         ans_num=input_ans_num)
...
def Delete_DynMCQanswer_view(request, input_q_num, input_ans_num):
    ...
    while answer_num <= nb_answers:
        tmp_answer = get_object_or_404(DynMCQanswer, q_num=input_q_num,
                                       ans_num=answer_num)
...
def pass_testslist_teacher_view(request):
    ...
    """
    pass_dynMCQtest_all = PassDynMCQTestInfo.objects.all()
...
def tests_list_student_view(request):
    ...
    # We compare the groups of each test with the groups of the user
    # to check if the user can pass the test
...
def launch_specific_dynmcq_view(request, input_id_test):
    """Function to launch a test in order to get the test available
    for users in a specific group
    ...
    # Same code for method POST
    if request.method == 'POST':
        form = DynMCQTestInfoFormlaunch(request.POST, instance=DynMCQTestInfo)
        choices = []
        for group in group_names:
            list = []
            list.append(group.name)
    ...
    return render(request, 'manage_tests/launch_specific_dynmcq_test.html',
                  context)
...
def in_launch_specific_dynmcq_view(request, input_id_test):
    ...
    return render(request, 'manage_tests/in_launch_specific_dynmcq_test.html',
                  context)
