...
from .forms import DynMCQquestionForm, DynMCQanswerForm, \
	Pass_DynMCQTestForm, DynMCQquestionForm_question, DynMCQTestInfoForm, \
	DynMCQTestInfoForm_questions, Question_difficulty_form, \
	MCQQuestion_difficulty_form, DynMCQTestInfoForm_launch, DynquestionForm, \
	Pass_DynquestionTestForm
...
def DynMCQquestion_select_menu_view(request, input_id_test):
    ...
    if len(DynMCQquestionTestList) == 0 and len(DynquestionTestList) == 0:
        empty = True
        QuestionsSet = formset_factory(DynMCQTestInfoForm_questions,
                                       extra=2)  # Two form for both question types
...
def Add_difficulty_view(request, input_q_num):
    ...
    for theme in form:
        ...
        for i in range(5):
            ...
            choices.append(list)
        theme.fields[
            'difficulty'].choices = choices  # Filling the choices for theme/difficulty as [theme,difficulty]
...
def DynMCQTest_pass_menu_view(request, input_id_test):
    ...
    if nb_pass_test > 0:
        ...
        if PassDynMCQTest.objects.filter(id_test=input_id_test,
                                         ...
                                         attempt=nb_pass_test).exists() or PassDynQuestionTest.objects.filter(
                id_test=input_id_test, id_student=request.user.username,
...
def DynMCQtest_pass_view(request, input_id_test, input_id_student,
    ...
    if (nb_mcq_questions > 0):
        ...
        PassDynMCQTestSet = formset_factory(Pass_DynMCQTestForm,
        ...
        for ans in form_mcq_answers:
            ...
            for answer in DynMCQanswers:
                ...
                choices.append(list)
            ans.fields[
                'r_ans'].choices = choices  # Filling the choices as list of [ans_num,ans_text]
            i += 1
    ...
    if (nb_normal_questions > 0):
        ...
        PassDynquestionTestSet = formset_factory(Pass_DynquestionTestForm,
...
def Edit_DynMCQquestion_view(request, input_q_num):
    ...
    form = DynMCQquestionForm_question(request.POST,
